...
flag_status usart_interrupt_flag_get(uint32_t usart_periph, uint32_t int_flag)
{
...
static int usart_at32_init(const struct device *dev)
{
	...
	crm_periph_clock_enable(cfg->rcu_periph_clock, true);
	usart_reset(cfg->reg);
	usart_init(cfg->reg, data->baud_rate, word_length, USART_STOP_1_BIT);
	usart_parity_selection_config((usart_type *)cfg->reg, parity);
	usart_receiver_enable(cfg->reg, TRUE);
	usart_transmitter_enable(cfg->reg, TRUE);
	usart_enable(cfg->reg, TRUE);
...
static int usart_at32_poll_in(const struct device *dev, unsigned char *c)
{
	...
	status = usart_flag_get(cfg->reg, USART_RDBF_FLAG);
...
static void usart_at32_poll_out(const struct device *dev, unsigned char c)
{
	...
	while (usart_flag_get(cfg->reg, USART_TDBE_FLAG) == RESET) {
		;
	}
...
static int usart_at32_err_check(const struct device *dev)
{
	...
	if (status & USART_ROERR_FLAG) {
		usart_flag_clear(cfg->reg, USART_ROERR_FLAG);

		errors |= UART_ERROR_OVERRUN;
	}

	if (status & USART_PERR_FLAG) {
		usart_flag_clear(cfg->reg, USART_PERR_FLAG);

		errors |= UART_ERROR_PARITY;
	}

	if (status & USART_FERR_FLAG) {
		usart_flag_clear(cfg->reg, USART_FERR_FLAG);

		errors |= UART_ERROR_FRAMING;
	}

	usart_flag_clear(cfg->reg, USART_NERR_FLAG);

	return errors;
}

#ifdef CONFIG_UART_INTERRUPT_DRIVEN
int usart_at32_fifo_fill(const struct device *dev, const uint8_t *tx_data, int len)
{
	const struct at32_usart_config *const cfg = dev->config;
	uint8_t num_tx = 0U;

	while ((len - num_tx > 0) && usart_flag_get(cfg->reg, USART_TDBE_FLAG)) {
...
int usart_at32_fifo_read(const struct device *dev, uint8_t *rx_data, const int size)
{
	const struct at32_usart_config *const cfg = dev->config;
	uint8_t num_rx = 0U;

	while ((size - num_rx > 0) && usart_flag_get(cfg->reg, USART_RDBF_FLAG)) {
...
void usart_at32_irq_tx_enable(const struct device *dev)
{
	...
	usart_interrupt_enable(cfg->reg, USART_TDC_INT, TRUE);
}
...
void usart_at32_irq_tx_disable(const struct device *dev)
{
	...
	usart_interrupt_enable(cfg->reg, USART_TDC_INT, FALSE);
}
...
int usart_at32_irq_tx_ready(const struct device *dev)
{
	...
	return usart_flag_get(cfg->reg, USART_TDBE_FLAG) &&
	       usart_interrupt_flag_get(cfg->reg, USART_TDC_INT);
}
...
int usart_at32_irq_tx_complete(const struct device *dev)
{
	...
	return usart_flag_get(cfg->reg, USART_TDC_FLAG);
}
...
void usart_at32_irq_rx_enable(const struct device *dev)
{
	...
	usart_interrupt_enable(cfg->reg, USART_RDBF_INT, TRUE);
}
...
void usart_at32_irq_rx_disable(const struct device *dev)
{
	...
	usart_interrupt_enable(cfg->reg, USART_RDBF_INT, FALSE);
}
...
int usart_at32_irq_rx_ready(const struct device *dev)
{
	...
	return usart_flag_get(cfg->reg, USART_RDBF_FLAG);
}
...
void usart_at32_irq_err_enable(const struct device *dev)
{
	...
	usart_interrupt_enable(cfg->reg, USART_ERR_INT, TRUE);
	usart_interrupt_enable(cfg->reg, USART_PERR_INT, TRUE);
}
...
void usart_at32_irq_err_disable(const struct device *dev)
{
	...
	usart_interrupt_enable(cfg->reg, USART_ERR_INT, FALSE);
	usart_interrupt_enable(cfg->reg, USART_PERR_INT, FALSE);
}
...
int usart_at32_irq_is_pending(const struct device *dev)
{
	...
	return ((usart_flag_get(cfg->reg, USART_RDBF_FLAG) &&
		 usart_interrupt_flag_get(cfg->reg, USART_RDBF_INT)) ||
		(usart_flag_get(cfg->reg, USART_TDC_FLAG) &&
		 usart_interrupt_flag_get(cfg->reg, USART_TDC_INT)));
}

void usart_at32_irq_callback_set(const struct device *dev, uart_irq_callback_user_data_t cb,
				 void *user_data)
{
	struct at32_usart_data *const data = dev->data;
...
#define AT32_USART_IRQ_HANDLER(n)                                                                  \
	...
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), usart_at32_isr,             \
			    DEVICE_DT_INST_GET(n), 0);                                             \
		irq_enable(DT_INST_IRQN(n));                                                       \
	}
#define AT32_USART_IRQ_HANDLER_FUNC_INIT(n) .irq_config_func = usart_at32_config_func_##n
...
#define AT32_USART_INIT(n)                                                                         \
	...
	static const struct at32_usart_config usart_at32_config_##n = {                            \
		.reg = (usart_type *)DT_INST_REG_ADDR(n),                                          \
		.rcu_periph_clock = DT_INST_PROP(n, rcu_periph_clock),                             \
		.pcfg = PINCTRL_DT_INST_DEV_CONFIG_GET(n),                                         \
		.parity = DT_INST_ENUM_IDX_OR(n, parity, USART_PARITY_NONE),                       \
		AT32_USART_IRQ_HANDLER_FUNC_INIT(n)};                                              \
	DEVICE_DT_INST_DEFINE(n, &usart_at32_init, NULL, &usart_at32_data_##n,                     \
			      &usart_at32_config_##n, PRE_KERNEL_1, CONFIG_SERIAL_INIT_PRIORITY,   \
